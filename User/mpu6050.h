...
extern MPU6050_TypeDef MPU6050[3];

void get_total_angle_MPU6050(MPU6050_TypeDef *p,int i);

uint8_t MPU_Init(IIC_TypeDef* IIC); 								//初始化MPU6050
uint8_t MPU_Write_Len(IIC_TypeDef* IIC,uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf);//IIC连续写
uint8_t MPU_Read_Len(IIC_TypeDef* IIC,uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf); //IIC连续读
uint8_t MPU_Write_Byte(IIC_TypeDef* IIC,uint8_t reg,uint8_t data);				//IIC写一个字节
uint8_t MPU_Read_Byte(IIC_TypeDef* IIC,uint8_t reg);						//IIC读一个字节
...
uint8_t MPU_Set_Fifo(IIC_TypeDef* IIC,uint8_t sens);


void MPU_Read_Init(void);
